...
function initCloudant() {
    var cloudantURL = appEnv.services.cloudantNoSQLDB[0].credentials.url || appEnv.getServiceCreds("clinical-desc-db").url;
...
function initConversation() {
    ...
    var conversationCredentials = appEnv.getServiceCreds("clinical-desc-conversation");
...
var chatbot = {
    sendMessage: function(req, callback) {
        var userRecord = req.session.userRecord;
        ...
        buildContextObject(req, function(err, params) {
            ...
            if (params.message) {
                ...
                conversation.message(params, function(err, data) {
                    ...
                    console.log("Got response from DESC AI: ", JSON.stringify(data));

                    updateContextObject(data, userRecord, function(err, res) {
...
function buildContextObject(req, callback) {
    ...
    if (req.session.userRecord) {
        userRecord = req.session.userRecord;
    }
        ...
        if (context.claim_step === "amount") {
            ...
            if (isNaN(userDate)) {
                ...
                if (userDate.getTime() > cDate.getTime()) {
                    ...
                    return callback(null, reprompt);
                } else { // Otherwise format the date to YYYY-MM-DD - DESC AI will also verify
    ...
    if ((!message || message === '') && !context) {
        ...
        parseRecordTitles(userRecord, function(services, procedures) {
 ...
 */
function parseRecordTitles(doc, callback) {

    var records = doc.records;
    var currentService = '';
    var recordServices = [];
    var recordProcedures = [];
    var proc = [];

    records.forEach(function(record) {

        if (record.type === currentService) {
            proc.push(record.title);
        } else {
            currentService = record.type;
            recordServices.push(record.type);

            if (proc.length > 0) {
                recordProcedures.push(proc);
    ...
    return callback(recordServices, recordProcedures);
}
 ...
 */
function updateContextObject(response, userRecord, callback) {
    ...
    if (context.chosen_procedure) {
        ...
        var records = userRecord.records;
        ...
        for (var n = 0; n < records.length; n++) {
            ...
            if (records[n].title.toUpperCase() === procedure.toUpperCase()) {
                procedure_details = {
                    "limit": "$" + records[n].claimLimit,
                    "claimed": "$" + records[n].amountClaimed,
                    "coverage": records[n].percentCovered + "%",
                    "term": records[n].scope,
                    "start": records[n].startDate,
                    "end": records[n].endDate,
                    "code": records[n].code,
                    "claims": records[n].claims
                };
            }
        }
