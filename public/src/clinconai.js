...
function userMessage(message) {
    ...
    xhr.onload = function() {
        ...
        if (xhr.status === 200 && xhr.responseText) {
            ...
            console.log("Got response from DESC: ", JSON.stringify(response));
...
function displayMessage(text, user) {
    ...
    if (user === watson) {
      var name = "ClinconAI";
      bubble.innerHTML = "<div class='clinconaiTitle'>" + name + " | " + getTimestamp() + "</div><div class='clinconai'>" + text + "</div>";
    } else {
        var name = "John";
