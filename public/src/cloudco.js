...
function makeHistoryRow(claim) {
    ...
    row.innerHTML = '<div class="marker">' +
        ...
        '</div>' +
        '<div class="claimdata">' + claim.record + '</div>' +
...
function getClaims() {
    ...
    get('./history', function(reply) {
        ...
        var claimlist = document.getElementById('list');
...
function createBenefitRow(record) {
    var row = document.createElement('div');
    row.className = 'benefitrow';
    row.innerHTML = '<div class="benefiticon">' +
        '<img class="benefitimage" src="images/health/' + record.icon + '.svg">' +
        '</div>' +
        '</div>' +
        '<div class="benefitTitle">' + record.title + '</div>' +
        '<div id="' + record.title + '-ToggleTextOpen" class="benefitMenu">View Benefit<i style = "padding-left:10px" class="fa fa-angle-down" aria-hidden="true"></i></div>' +
        '<div id="' + record.title + '-ToggleTextClose" class="benefitMenu" style = "display:none">Close Benefit<i style = "padding-left:10px" class="fa fa-angle-up" aria-hidden="true"></i></div>';
    row.onclick = function() {
        toggleDetails(record.title);
    }
...
function createBenefitDetail(record) {

    var detail = document.createElement('div');
    detail.className = 'benefitdetail';
    detail.id = record.title;

    detail.innerHTML =
        '<div class="benefitfacts">' +
        '<div class="benefitfact">' +
        '<div class="factlabel">benefit</div>' +
        '<div class="factcheck">' + record.description + '</div>' +
        '</div>' +
        '<div class="benefitfact">' +
        '<div class="factlabel">limit</div>' +
        '<div class="factcheck">$' + record.claimLimit + '</div>' +
        '</div>' +
        '<div class="benefitfact">' +
        '<div class="factlabel">coverage</div>' +
        '<div class="factcheck">' + record.percentCovered + '%</div>' +
        '</div>' +
        '<div class="benefitfact">' +
        '<div class="factlabel">term</div>' +
        '<div class="factcheck">' + record.scope + '</div>' +
        '</div>' +
        '<div class="benefitfact">' +
        ...
        '<div class="factcheck">' + record.code + '</div>' +
        '</div>' +
        '</div>'
...
function getBenefits() {
    ...
    get('./healthBenefits', function(reply) {
        ...
        records.forEach(function(record) {
            ...
            if (recordAreas[record.type]) {
                recordAreas[record.type].push(record);
            } else {
                recordAreas[record.type] = [];
                recordAreas[record.type].push(record);
                recordKeys.push(record.type);
                var benefitTitle = document.createElement('div');
                benefitTitle.className = "benefitTypeTitle";
                benefitTitle.innerHTML = record.type + " Benefits";
                var benefitEntity = createBenefitEntity(record.type);
            ...
            var anchor = document.getElementById(record.type);

            var benefitRow = createBenefitRow(record);
            var benefitDetail = createBenefitDetail(record);

            anchor.appendChild(benefitRow);
            anchor.appendChild(benefitDetail);
        });

        var uniquebenefits = recordTitles.filter(unique); // returns ['a', 1, 2, '1']
