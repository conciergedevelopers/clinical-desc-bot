...
// Cloudant
var Logs, Benefits;
var cloudantURL = appEnv.services.cloudantNoSQLDB[0].credentials.url || appEnv.getServiceCreds("desc-registrar-db").url;
...
app.get('/history', isLoggedIn, function(req, res) {
    ...
    Benefits.find({ selector: {
        ...
        if (err) {
            ...
            doc.records.forEach(function(records) {

                if (record.claims.length > 0) {
                    record.claims.forEach(function(claim) {
                        var detailedclaim = {};
                        ...
                        detailedclaim.record = record.title;
                        detailedclaim.icon = record.icon;
        ...
        Benefits.find({ selector: {
            ...
            if (err) {
                ...
                doc.records.forEach(function(record) {
                    ...
                    if (record.title === claim.benefit) {
                        recordFound = true;

                        claim.outcome = 'DENIED';
                        claim.payment = 0;

                        var possibleEligibility = claim.amount * record.percentCovered / 100;

                        var amountAvailable = record.claimLimit - record.amountClaimed;
                        ...
                        if (amountAvailable <= 0) {
                            claim.outcome = 'NONE';
                            claim.payment = 0;
                            message = "Sorry, you reached your claim limit. So none of the amount could be covered by your bot.";
                        }

                        if (record.amountClaimed > amountAvailable && amountAvailable > 0) {
                            claim.outcome = 'PARTIAL';
                            claim.payment = amountAvailable;
                            record.amountClaimed = record.Limit;
                            message = "You have reached max coverage. Remaining $" + amountAvailable + " of clinical record limit applied.";
                        }

                        if (possibleEligibility < amountAvailable) {
                            claim.outcome = 'FULL';
                            claim.payment = possibleEligibility;
                            record.amountClaimed = record.amountClaimed + possibleEligibility;
...
app.get('/healthBenefits', isLoggedIn, function(req, res) {
    ...
    if (req.session.userRecord) {
        res.send(JSON.stringify(req.session.userRecord, null, 3));
    } else {
        getUserRecord(req, function(err, doc) {
            if (err) {
                res.send(err);
            } else {
...
function getUserRecord(req, callback) {
    //console.log(req);
    ...
    Benefits.find({ selector: {
        ...
        if (err) {
            ...
            var doc = result.docs[0];
            req.session.userRecord = doc;
            return callback(null, doc);
        } else {
            console.error("No user record found.");
            return callback("No user record found.");
        }
    });
}
...
// =====================================
// WATSON CONVERSATION FOR ClinCon AI =========
// =====================================
app.post('/api/clinconai', function(req, res) {
